...
class Instagram {
   ...
   * @return mixed
   */
  public function getUserMedia($id = 'self', $limit = 0, $max_id = 0) { //@@raine added max_id
    if ($max_id > 0) {
        return $this->_makeCall('users/' . $id . '/media/recent', true, array('count' => $limit, 'max_id' => $max_id)); //$raine added max_id parameter
    } else {
